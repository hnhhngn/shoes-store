...
function Shopcart(props) {
	...
	const changeQuantity = (e) => {
		...
		if (number > 100 || number == "" || number == 0) {
			alert("số lượng mua tạm thời max là 100");
			return false
		}
		...
		if ((prodetail.inventory - number) < 0) {
			alert("size này tạm thời chỉ còn " + prodetail.inventory + " sản phẩm!")
			return false
		}
	...
	const changeSize = (e) => {
		...
		// check trùng size
		if (prosame !== 0) {
			alert("size của sản phẩm này đã tồn tại trong giỏ hàng")
			return false
		}

		// check ton kho
		if ((prodetailnew.inventory - number) < 0) {
			alert("size này tạm thời chỉ còn " + prodetailnew.inventory + " sản phẩm!")
			return false
		}
		...
		return <tr>
			...
			<td>
				<div style={{ marginTop: 10 }} className="select-custom">
					<select style={{ width: 50 }} id="size" name={idsize} value={shopcart.productdetail.id || 1} className="form-control" onChange={(e) => changeSize(e)}>
