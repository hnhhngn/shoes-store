...
function ChartProduct() {
  ...
  const [nhaphang, setnhaphang] = useState(["số lượng: 0"])
  ...
  const getProductByName = () => {
    ...
    if (isAll === 0 && productdetailid !== "" && productdetailid > 0) {
      axios.get(hostname + '/api/repository/nhaphang?mode=detail&id=' + productdetailid + '&datefrom=' +
        finalDateFrom + '&dateto=' + finalDateto, config)
        .then((Response) => {
          console.log(Response.data)
          let arrdt = []
          let item = "số lượng:" + Response.data[0]
          arrdt.push(item)
          setnhaphang(arrdt)
          let arrln = []
          arrln.push(Response.data[1])
          setgianhap(arrln)
        })
        .catch((error) => {
      ...
      axios.get(hostname + '/api/repository/nhaphang?mode=product&id=' + proid + '&datefrom=' + finalDateFrom + '&dateto=' + finalDateto, config)
        .then((Response) => {
          console.log(Response.data)
          let arrdt = []
          let item = "số lượng:" + Response.data[0]
      ...
      axios.get(hostname + '/api/repository/nhaphang?mode=all&id=0&datefrom=' + finalDateFrom + '&dateto=' + finalDateto, config)
        .then((Response) => {
          console.log(Response.data)
          let arrdt = []
          let item = "số lượng:" + Response.data[0]
      ...
      <Page title="THỐNG KÊ SẢN PHẨM" breadcrumbs={[{ name: 'Chart Product', active: true }]}>
        ...
        <Row>
          <Col xl={10} lg={12} md={12} >
            <Card>
              <CardHeader>
                <p style={{ color: 'black' }}> Biểu Đồ Nhập Sản Phẩm </p>
              </CardHeader>
              <CardBody>
