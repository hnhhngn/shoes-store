...
function RepositoryAdmin() {
  ...
  //add res
  if (addRes !== "") {
    if (pagepresent == totalpage && repository.length < 8 && type == "PHIEUNHAP") {
      let temp = [];
  ...
  let jsx = listPageProduct.map((product, index) => {
    ...
    let info = listsize.map((size, ind) => {
      ...
      return <>
        {(size.inventory < 5) ?
          <tr style={{ backgroundColor: 'red' }} onClick={(e) => handleShow(res)} >
            ...
            <th style={{ color: '#212529', fontWeight: 400 }}>
              <p style={{ marginLeft: 30, color: '#212529', fontWeight: 400 }}>
                {size.inventory}
              </p>
            </th>
          </tr>
