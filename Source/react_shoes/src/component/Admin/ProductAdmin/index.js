...
function ProductAdmin() {
  ...
  if (updateProduct !== "") {
    setupdateProduct("")
    let list = [...products];
    list.filter((pro, index) => {
      if (pro.id === updateProduct.id) {
        if (updateProduct.status == statusPro) {
          console.log("vào trung")
          pro = { ...updateProduct }
        } else {
          console.log("vào k trung")
          list.splice(index, 1)
  ...
  let JsxContent = products.map((product, index) => {
    ...
    return <>
      <tr onClick={(e) => handleShowProper(product.id)} >
        ...
        <th style={{ color: '#212529', fontWeight: 400 }}>{formatCurrency(price)}</th>
  ...
  // add product
  if (addProducts.name != "") {
    if (pagepresent === totalpage && products.length < 8 && addProducts.status == statusPro) {
      let arr = []
      products.filter((pro) => {
        arr.push(pro)
      })
  ...
  const filterCategory = (e) => {
    ...
    axios.get(hostname + '/api/product/page?page=1&size=' + PageSize + '&category=' + e)
      .then((Response) => {
        const data = Response.data;
        settotalpage(data.totalpage)
        setpagepresent(data.page)
