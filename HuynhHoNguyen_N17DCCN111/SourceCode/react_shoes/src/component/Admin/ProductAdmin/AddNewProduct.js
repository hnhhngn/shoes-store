...
function AddNewProduct(props) {
    ...
    const createProduct = () => {
        ...
        if (names.length > 100 || names.length < 5 || names === "") {
            showMessage("ERR_PDNM_E001");
            return false;
        }

        if (mota.length > 500 || mota.length < 5 || mota === "") {
            showMessage("ERR_DES_E001");
            return false;
        }

        if (kq < 1000 || kq === "") {
            showMessage("ERR_PRICE_E001");
            return false;
        }

        if (unitypes === "") {
            showMessage("ERR_UNI_E001");
            return false;
        }
        ...
        axios.post(hostname + '/api/product', bodyParameters, header)
            .then((Response) => {
                showMessage("MSG_PRODUCT_SUCCESS");
