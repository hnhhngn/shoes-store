...
function EditRepository(props) {
    ...
    const findProduct = () => {
        ...
        axios.get(hostname + `/api/product/id/${id}`)
            ...
            .catch((error) => {
                alert("mã sản phẩm không chính xác")
                console.log("error")
            })
    }
    ...
    var jsxAdd = <Form>
        ...
        <FormGroup row>
            <Label style={{ color: 'black', fontWeight: 400 }} for="exampleText" sm={2}>Chọn Sản Phẩm</Label>
        </FormGroup>
        <FormGroup row>
            <Label style={{ color: 'black', fontWeight: 400 }} for="exampleText" sm={2}>Mã Sản Phẩm</Label>
            <Col><Input type="text" id="id" style={{ width: 500, fontSize: 16 }} name="id" autoComplete='off' required /></Col>
            <Col>
                <Button variant="success" onClick={findProduct}>
                    Tra Cứu
                </Button>
            </Col>
        </FormGroup>
